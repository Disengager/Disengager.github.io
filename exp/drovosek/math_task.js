function load_biography(dt) {
	window.biography = dt;
	if(window.biography.sequence != undefined)
		window.biography.sequence = window.biography.sequence * 1;
}

function is_skip_animation(param1){
	if(window.is_skip(param1)) return true;

	if(!window.settings.is_animation) return true;
...
window.settings = {
	is_animation: true,
	pose: 'left',
	pose_status: ['left', 'right', 'null'],
	game_over: false, 
	response: '',

	get_pose: function(param1, param2) {
		if(this.pose == this.pose_status[0])
			return sequence.sex[param2] + '_l_' + param1 + '.jpg'
		return sequence.sex[param2] + '_r_' + param1 + '.jpg'
	},
	switch_pose: function() { //переключение позы, принимает на вход кнопку которую нажал испытуемый
		if(this.pose == this.response) 
			return false;

		this.pose = this.pose == this.pose_status[0] ? this.pose_status[1] : this.pose_status[0]					
	},
	get_log: function(number) {
		let log = window.sequence.target[number];
		if(log == undefined) return 'log_0.png';

		let log_link = 'log_' + log[1];
		
		if(log[0] == this.pose_status[2]) return log_link + '.png';

		log_link += log[0] == this.pose_status[0] ? '_l' : '_r';
		return log_link + '.png';
	},
	crash_check: function(index) { //переключение позы, принимает на вход кнопку которую нажал испытуемый
		console.log(sequence.target[index][0]);
		console.log(this.response);
		console.log(sequence.target[index][0] == this.response);
		return sequence.target[index][0] == this.response; //позиция куска == позиции персонажа
	},
}

function load_game() {
	let l = 'left', r = 'right', n = 'null';
	console.log(window.biography.sequence);
	console.log(typeof(window.biography.sequence));
	window.sequence = {
		completed: 0,
		next: function() {
			this.target.shift();
		...
		sex: [], 
		load_sex: function() {
			this.sex = this.sex_base[window.biography.sequence]
		}
	}
	sequence.load_sex();
}
